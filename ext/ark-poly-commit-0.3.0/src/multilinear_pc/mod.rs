...
impl<E: PairingEngine> MultilinearPC<E> {
    ...
    pub fn commit(
        ...
        let scalars: Vec<_> = polynomial
            ...
            .collect();
	    //GRT modify
        //     let g_product = VariableBaseMSM::multi_scalar_mul(&ck.powers_of_g[0], scalars.as_slice()).into_affine();
        let g_product = msm(&ck.powers_of_g[0], scalars.as_slice()).into_affine();
    ...
    pub fn open(
        ...
        for i in 0..nv {
            ...
            let scalars: Vec<_> = (0..(1 << k))
                ...
                .collect();
	        //GRT modify
            // let pi_h = VariableBaseMSM::multi_scalar_mul(&ck.powers_of_h[i], &scalars).into_affine(); // no need to move outside and partition
            let pi_h = msm(&ck.powers_of_h[i], &scalars).into_affine(); // no need to move outside and partition
            proofs.push(pi_h);
        }
