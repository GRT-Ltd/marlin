    ...
    for<'a, 'b> &'a P: Div<&'b P, Output = P>,
{
    ...
    pub fn commit(
        ...
        let mut commitment = msm(&powers.powers_of_g[num_leading_zeros..], &plain_coeffs);
        end_timer!(msm_time);
	    ...
	    //GRT modify
        //let random_commitment = VariableBaseMSM::multi_scalar_mul(&powers.powers_of_gamma_g, random_ints.as_slice()).into_affine();
        let random_commitment = msm(&powers.powers_of_gamma_g, random_ints.as_slice()).into_affine();
    ...
    pub(crate) fn open_with_witness_polynomial<'a>(
        ...
        let mut w = msm(&powers.powers_of_g[num_leading_zeros..], &witness_coeffs);
        ...
        let random_v = if let Some(hiding_witness_polynomial) = hiding_witness_polynomial {
            ...
            // GRT modiy
            //     let temp = VariableBaseMSM::multi_scalar_mul(&powers.powers_of_gamma_g, &random_witness_coeffs);
            let temp = msm(&powers.powers_of_gamma_g, &random_witness_coeffs);
            w += &temp;
            end_timer!(witness_comm_time);
