    ...
    P::Point: Index<usize, Output = E::Fr>,
{
        ...
        P: 'a,
    {
        ...
        for p in polynomials {
            ...
            let mut commitment = msm(&powers_of_g, &plain_ints);
            end_timer!(msm_time);
            ...
            //GRT modify
            //     let random_commitment = VariableBaseMSM::multi_scalar_mul(&powers_of_gamma_g, &random_ints).into_affine();
            let random_commitment = msm(&powers_of_gamma_g, &random_ints).into_affine();
            end_timer!(msm_time);
        ...
        Self::Commitment: 'a,
    {
        ...
        let mut w = witnesses
            ...
            .map(|w| {
                ...
                // Compute MSM
		        //GRT modify
                // let res =  VariableBaseMSM::multi_scalar_mul(&powers_of_g, &witness_ints);
                let res = msm(&powers_of_g, &witness_ints);
                res
            })
            .collect::<Vec<_>>();
        ...
        let random_v = if let Some(hiding_witnesses) = hiding_witnesses {
            ...
            ark_std::cfg_iter_mut!(w)
                ...
                .for_each(|(i, witness)| {
                    ...
                    //GRT modify
                    // let temp = VariableBaseMSM::multi_scalar_mul(&powers_of_gamma_g, &hiding_witness_ints);
                    let temp = msm(&powers_of_gamma_g, &hiding_witness_ints);
                    *witness += &temp;
                });
            end_timer!(witness_comm_time);
