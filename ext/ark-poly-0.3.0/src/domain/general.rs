...
impl<F: FftField> EvaluationDomain<F> for GeneralEvaluationDomain<F> {
    ...
    fn fft_in_place<T: DomainCoeff<F>>(&self, coeffs: &mut Vec<T>) {
        ...
        let grt_time = start_timer!(|| format!("fft {} len = {}",str , coeffs.len()));
        coeffs.resize(self.size(), T::zero());
        #[cfg(not(feature = "cuda"))]
        map!(self, fft_in_place, coeffs);
        #[cfg(feature = "cuda")]
        ntt_cuda::NTT(DEFAULT_GPU, coeffs, NTTInputOutputOrder::NN);
        end_timer!(grt_time);
    }
    ...
    fn ifft_in_place<T: DomainCoeff<F>>(&self, evals: &mut Vec<T>) {
        ...
        let grt_time = start_timer!(|| format!("ifft {} len = {}",str , evals.len()));
        evals.resize(self.size(), T::zero());
        #[cfg(not(feature = "cuda"))]
        map!(self, ifft_in_place, evals);
        #[cfg(feature = "cuda")]
        ntt_cuda::iNTT(DEFAULT_GPU, evals, NTTInputOutputOrder::NN);
        end_timer!(grt_time);
    }
    ...
    fn coset_fft_in_place<T: DomainCoeff<F>>(&self, coeffs: &mut Vec<T>) {
        ...
        let grt_time = start_timer!(|| format!("coset_fft {} len = {}",str , coeffs.len()));
        coeffs.resize(self.size(), T::zero());
        #[cfg(not(feature = "cuda"))]
        map!(self, coset_fft_in_place, coeffs);
        #[cfg(feature = "cuda")]
        ntt_cuda::coset_NTT(DEFAULT_GPU, coeffs, NTTInputOutputOrder::NN);
        end_timer!(grt_time);
    ...
    fn coset_ifft_in_place<T: DomainCoeff<F>>(&self, evals: &mut Vec<T>) {
        ...
        let grt_time = start_timer!(|| format!("coset_ifft {} len = {}",str , evals.len()));
        evals.resize(self.size(), T::zero());
        #[cfg(not(feature = "cuda"))]
        map!(self, coset_ifft_in_place, evals);
        #[cfg(feature = "cuda")]
        ntt_cuda::coset_iNTT(DEFAULT_GPU, evals, NTTInputOutputOrder::NN);
        end_timer!(grt_time);
    }
