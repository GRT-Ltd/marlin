...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function(name, |b| {
        b.iter(|| {
            ...
            let res_gpu = multi_scalar_mult_arkworks(&points.as_slice(), unsafe {
                std::mem::transmute::<&[_], &[BigInteger256]>(
                    scalars.as_slice(),
                )
            });
            println!("GRT G1 gpu time = {:?}", start.elapsed());

            let start = Instant::now();
            let res_cpu = VariableBaseMSM::multi_scalar_mul(&points.as_slice(), unsafe {
                std::mem::transmute::<&[_], &[BigInteger256]>(
                    scalars.as_slice(),
                )
            });
            println!("GRT G1 cpu time = {:?}", start.elapsed());
            assert_eq!(res_gpu, res_cpu);
        })
    });
...
fn criterion_benchmark_fp2(c: &mut Criterion) {
	...
	// GRT modify
            let start = Instant::now();
            let res_gpu = multi_scalar_mult_fp2_arkworks(&points.as_slice(), unsafe {
                std::mem::transmute::<&[_], &[BigInteger256]>(
                    scalars.as_slice(),
                )
            });
            println!("GRT G2 gpu time = {:?}", start.elapsed());
            ...
            println!("GRT G2 cpu time = {:?}", start.elapsed());
